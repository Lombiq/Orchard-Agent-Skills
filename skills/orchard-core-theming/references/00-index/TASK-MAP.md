# Task Map

Map common tasks to the exact docs to read.

## Override a content item display
- Read: `20-shapes-placement/SHAPES.md`
- Read: `20-shapes-placement/ALTERNATES.md`
- Read: `20-shapes-placement/PLACEMENT.md`

## Render BagPart/FlowPart/ListPart items
- Read: `40-content-model/CONTAINERS.md`
- Read: `30-templating/LIQUID-SHAPES.md`
- Read: `20-shapes-placement/PLACEMENT.md`

## Build/override a shape (template in the Views folder)
- Read: `20-shapes-placement/SHAPE-WORKFLOW.md`
- Read: `20-shapes-placement/ALTERNATES.md`
- Read: `20-shapes-placement/PLACEMENT.md`
- Read: `40-content-model/FIELDS.md`
- Read: `30-templating/TAG-HELPERS.md` and `30-templating/ORCHARD-HELPER.md`

## Build/override a Content shape template, optionally with a custom stereotype (e.g., Widget, Section, or Block)
- Read: `20-shapes-placement/SHAPE-WORKFLOW.md`
- Read: `20-shapes-placement/ALTERNATES.md`
- Read: `40-content-model/CONTENT-DEFINITIONS.md`
- Read: `40-content-model/CONTAINERS.md` (BagPart/FlowPart)

## Understand content item structure
- Read: `40-content-model/CONTENT-ITEMS.md`
- Read: `40-content-model/CONTENT-DEFINITIONS.md`
- Read: `40-content-model/FIELDS.md`
- Read: `40-content-model/PARTS.md`
- Read: `40-content-model/SETTINGS.md`

## Create or read recipes
- Read: `60-recipes/README.md`
- Read: `60-recipes/RECIPE-STEPS.md`
- Read: `60-recipes/RECIPE-CONTENT.md`
- Read: `60-recipes/RECIPE-COMMANDS.md`

## Add theme scripts/styles
- Read: `50-assets-resources/RESOURCES.md`
- Read: `10-theme-basics/THEME-MANIFEST.md`
- Read: `30-templating/TAG-HELPERS.md`

## Create a theme or module
- Read: `10-theme-basics/CREATE-THEME.md`
- Read: `10-theme-basics/CREATE-MODULE.md`

## Use tag helpers in Razor
- Read: `30-templating/TAG-HELPERS.md`

## Use Liquid in shape templates
- Read: `30-templating/LIQUID.md`
- Read: `30-templating/LIQUID-FILTERS.md`
- Read: `30-templating/LIQUID-TAGS.md`
- Read: `30-templating/LIQUID-SHAPES.md`

## Find shape alternates
- Read: `20-shapes-placement/ALTERNATES.md`
- Read: `70-debugging-discovery/SHAPE-TRACE.md`

## Theme structure or layout
- Read: `10-theme-basics/THEME-STRUCTURE.md`
- Read: `10-theme-basics/LAYOUTS-ZONES.md`

## Understand solution structure
- Read: `10-theme-basics/SOLUTION-STRUCTURE.md`
