# Task Map

Start here. Pick the task and open only the referenced files.

## Determine template language (Razor vs Liquid)
- Read: `10-understand-structure/SOLUTION-STRUCTURE.md`
- Read: `10-understand-structure/THEME-STRUCTURE.md`
- If the theme uses `.cshtml`, use `30-razor/INDEX.md`.
- If the theme uses `.liquid`, use `40-liquid/INDEX.md`.

## Add or update a content type/part/field in ContentDefinition.json
- Prefer the extractor: `50-content-model/CONTENT-DEFINITIONS-EXTRACTOR.md` (pull the existing slice before editing).
- Read: `50-content-model/CONTENT-DEFINITIONS.md`
- Read: `50-content-model/CONTENT-DEFINITIONS-EXAMPLES.md` (sample patterns)
- Read: `50-content-model/FIELDS.md`
- Read: `50-content-model/SETTINGS.md`
- Read: `50-content-model/PARTS.md`

## Extract a focused content definition slice (large JSON)
- Read: `50-content-model/CONTENT-DEFINITIONS-EXTRACTOR.md`

## Create a setup recipe
- Read: `70-recipes/RECIPE-STEPS-CORE.md` (features/themes/settings/recipes)
- Read: `70-recipes/RECIPE-STEPS-DEFINITIONS.md` (content types/parts/fields)
- Read: `70-recipes/FEATURE-CATALOG.md`
- Read: `70-recipes/BASE-RECIPES.md`
- Read: `70-recipes/RECIPE-EXAMPLES-SETUP.md` (sample setup recipes)

## Add content types and sample content to a recipe
- Read: `70-recipes/RECIPE-CONTENT.md`
- Read: `70-recipes/ID-GENERATION.md`
- Read: `70-recipes/RECIPE-EXAMPLES-CONTENT.md`
- Read: `70-recipes/RECIPE-STEPS-DEFINITIONS.md`
- Read: `70-recipes/RECIPE-STEPS-CONTENT-MEDIA.md`
- Read: `50-content-model/CONTENT-DEFINITIONS.md`
- Read: `50-content-model/CONTENT-DEFINITIONS-EXAMPLES.md` (sample patterns)

## Create or update workflows in a recipe
- Read: `70-recipes/RECIPE-STEPS-TEMPLATES-WORKFLOWS.md`
- Read: `70-recipes/WORKFLOWS.md`
- Read: `70-recipes/RECIPE-EXAMPLES-WORKFLOWS.md`
- Read: `70-recipes/ID-GENERATION.md`

## Create or override a content item shape template
- Read: `20-shapes-placement/SHAPE-WORKFLOW.md`
- Read: `20-shapes-placement/ALTERNATES.md`
- Read: `50-content-model/FIELDS.md`
- Read: `50-content-model/CONTAINERS.md`
- Then choose Razor or Liquid: `30-razor/INDEX.md` or `40-liquid/INDEX.md`.

## Implement or override OrchardCore.Forms widgets and Form content
- Read: `20-shapes-placement/FORMS-WIDGETS.md`
- Read: `50-content-model/CONTAINERS.md`
- Read: `70-recipes/RECIPE-CONTENT.md`
- Read: `70-recipes/RECIPE-STEPS-CORE.md`

## Inspect real content items (SQLite)
- Read: `50-content-model/CONTENT-ITEMS-EXTRACTOR.md`

## Update a shape after adding fields
- Read: `50-content-model/FIELDS.md`
- Read: `20-shapes-placement/SHAPE-WORKFLOW.md`

## Render BagPart/FlowPart/ListPart items
- Read: `50-content-model/CONTAINERS.md`
- Read: `20-shapes-placement/PLACEMENT.md`
- Read: `40-liquid/LIQUID-SHAPES.md` (Liquid)
- Read: `30-razor/TAG-HELPERS-SHAPES.md` (Razor)

## Add scripts/styles and include them in the layout
- Read: `60-assets-resources/RESOURCES.md`
- Read: `60-assets-resources/STATIC-FILES.md`
- Read: `10-understand-structure/LAYOUTS-ZONES.md`
- Read: `30-razor/TAG-HELPERS-RESOURCES.md` (Razor examples)

## Find shape alternates and placement rules
- Read: `20-shapes-placement/ALTERNATES.md`
- Read: `20-shapes-placement/PLACEMENT.md`
- Read: `80-debugging-discovery/SHAPE-TRACE.md`

## Find evidence in Orchard Core source
- Read: `80-debugging-discovery/SOURCE-DISCOVERY.md`

## Theme structure or layout
- Read: `10-understand-structure/THEME-STRUCTURE.md`
- Read: `10-understand-structure/LAYOUTS-ZONES.md`

## Understand solution structure
- Read: `10-understand-structure/SOLUTION-STRUCTURE.md`
